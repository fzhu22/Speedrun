...
- Method (documented, authoritative): [docs/score-mapping.md](docs/score-mapping.md). Each covered section's held-out performance accuracy maps linearly to its section band [118,132]; the four sections sum to [472,528]; uncovered sections (e.g. CARS, out of scope) are **imputed at the covered-section mean** (a stated assumption, so the total stays on-scale); the **range** is the summed per-section Wilson bounds. Implemented in `computeProjected` ([ts/routes/speedrun/+page.svelte](ts/routes/speedrun/+page.svelte)) and reproduced offline by [testdeck/score_mapping.py](testdeck/score_mapping.py) (artifact `docs/eval-artifacts/score-mapping.json`).
- Never a point estimate alone; never a blended percentage. This is a display-layer index derived from the validated Performance score; it is **not** anchored to real MCAT scores (that is Step 4, out of scope).
...
- **Readiness mapping:** RESOLVED - the method is documented and implemented (per-section accuracy -> [118,132] band -> summed to [472,528], uncovered sections imputed at the covered mean, range from summed Wilson bounds; see [docs/score-mapping.md](docs/score-mapping.md)). Still open: anchoring/calibrating it against real practice-test scores (the bonus Step 4), which needs longitudinal student data.
