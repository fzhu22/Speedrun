...
TopicMastery = stats_pb2.TopicMasteryResponse
SpeedrunDashboard = speedrun_pb2.SpeedrunDashboardResponse
SpeedrunRecordReview = speedrun_pb2.SpeedrunRecordReviewResponse
SpeedrunShouldPrompt = speedrun_pb2.SpeedrunShouldPromptResponse
...
class Collection(DeprecatedNamesMixin):
    ...
    def topic_mastery(
        ...
        *,
        search: str = "",
        tag_prefix: str = "",
        mastered_retrievability: float = 0.0,
        include_descendants: bool = False,
        min_cards_for_average: int = 0,
    ) -> TopicMastery:
        """Speedrun: per-topic (tag) mastered-card counts and average FSRS recall.

        Read-only and computed in the Rust engine in a single pass, so it is fast
        enough to power the dashboard/coverage map on large collections."""
        return self._backend.topic_mastery(
            search=search,
            tag_prefix=tag_prefix,
            mastered_retrievability=mastered_retrievability,
            include_descendants=include_descendants,
            min_cards_for_average=min_cards_for_average,
        )
    ...
    def speedrun_dashboard(
        ...
        *,
        tag_prefix: str = "",
        mastered_retrievability: float = 0.0,
        give_up_line: float = 0.0,
        min_reviews_for_readiness: int = 0,
    ) -> SpeedrunDashboard:
        """Speedrun: read-only data for the readiness dashboard (coverage map,
        per-section memory, the three never-blended scores, and the
        prerequisite-aware study plan).

        Computed in the shared Rust engine so the desktop and AnkiDroid render an
        identical dashboard. Fields left at 0/empty fall back to their defaults
        (tag_prefix "MCAT", mastered_retrievability 0.9, give_up_line 0.5,
        min_reviews_for_readiness 200)."""
        return self._backend.speedrun_dashboard(
            tag_prefix=tag_prefix,
            mastered_retrievability=mastered_retrievability,
            give_up_line=give_up_line,
            min_reviews_for_readiness=min_reviews_for_readiness,
        )

    def speedrun_record_review(
        self, *, card_id: CardId, rating: int
    ) -> SpeedrunRecordReview:
        """Speedrun: update the per-family support-fading ladder for one answered
        card and persist the resulting rung (config map + a syncable
        ``speedrun_rung::Lx`` tag). Computed in the shared Rust engine so desktop
        and AnkiDroid update the ladder identically. ``rating`` is 1=Again,
        2=Hard, 3=Good, 4=Easy; success is ``rating >= 2``. Declarative cards are
        excluded, and the family is keyed by the AAMC content-category code the
        dashboard reads (so the two always agree)."""
        return self._backend.speedrun_record_review(card_id=card_id, rating=rating)

    def speedrun_should_prompt_disconfirmer(
        self, *, card_id: CardId, rating: int, session_misses: int = 0
    ) -> SpeedrunShouldPrompt:
        """Speedrun: whether a missed card should prompt for a disconfirmer, and
        whether the student is clearly struggling (misses = card.lapses +
        session_misses)."""
        return self._backend.speedrun_should_prompt_disconfirmer(
            card_id=card_id, rating=rating, session_misses=session_misses
        )

    def speedrun_validate_disconfirmer(self, *, text: str, answer: str = "") -> str:
        """Speedrun: validate a student-authored disconfirmer. Returns a revise
        message, or "" when it is acceptable (a nudge, not a grader)."""
        return self._backend.speedrun_validate_disconfirmer(text=text, answer=answer)

    def speedrun_create_disconfirmer(
        self,
        *,
        card_id: CardId,
        disconfirmer: str,
        ...
        """Speedrun: build a Speedrun Disconfirmer note from a missed card (the
        miss -> card loop). The card's front/back become the cover-story/answer,
        the new card lands in the Speedrun Disconfirmers deck, and the original is
        tagged so it isn't prompted again. Returns the new note id."""
        return NoteId(
            self._backend.speedrun_create_disconfirmer(
                card_id=card_id,
    ...
    def speedrun_ensure_notetypes(self) -> None:
        """Speedrun: create the Speedrun Disconfirmer + Pretest note types if
