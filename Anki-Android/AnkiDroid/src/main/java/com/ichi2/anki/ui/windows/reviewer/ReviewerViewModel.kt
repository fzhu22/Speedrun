...
import com.ichi2.anki.speedrun.SpeedrunAnsweredCard
import com.ichi2.anki.speedrun.SpeedrunReview
...
class ReviewerViewModel(
    ...
    /**
     * Emitted after a card is graded so [com.ichi2.anki.ui.windows.reviewer.ReviewerFragment]
     * (which has an `Activity`) can run the shared [SpeedrunReview.onCardAnswered] hook.
     * Buffered with [BufferOverflow.DROP_OLDEST] so grading is never coupled to Speedrun latency.
     */
    val speedrunAnsweredFlow =
        MutableSharedFlow<SpeedrunAnsweredCard>(
    ...
    init {
        ...
        // Speedrun: make sure the Speedrun note types exist on device (idempotent, defensive)
        launchCatchingIO { SpeedrunReview.ensureNotetypes() }
        launchCatchingIO {
    ...
    private suspend fun answerCardInternal(rating: Rating) {
        ...
        // Speedrun: hand the just-answered card to the fragment, which runs the shared hook
        speedrunAnsweredFlow.emit(SpeedrunAnsweredCard(card.id, rating))
